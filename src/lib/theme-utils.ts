...
/**
 * 将 HSL 值转换为 CSS 变量格式（仅返回数值部分）
 * 例如：[0, 0, 100] -> "0 0% 100%"
 */
function hslToCssValue(hsl: HSLValue | null): string {
  if (!hsl) {
    return "0 0% 0%" // 默认黑色
  }
  const [h, s, l] = hsl
...
export function applyThemeColors(colors: CustomThemeColors): void {
  ...
  Object.entries(colors.light || {}).forEach(([key, value]) => {
    ...
    if (value) {
      // 将 Hex 转换为 HSL 然后设置 CSS 变量
      const hsl = hexToHsl(value)
      root.style.setProperty(cssVar, hslToCssValue(hsl))
    } else {
  ...
  Object.entries(colors.dark || {}).forEach(([key, value]) => {
    ...
    if (value) {
      // 将 Hex 转换为 HSL 然后设置 CSS 变量
      const hsl = hexToHsl(value)
      darkCssRules += `  ${cssVar}: ${hslToCssValue(hsl)};\n`
    }
 ...
 * 将颜色转换为 HSL 格式
 */
export function hexToHsl(hex: string | HSLValue | null): HSLValue | null {
  // 如果输入已经是 HSLValue 类型，直接返回
  if (Array.isArray(hex)) {
    return hex
  }

  // 如果输入为 null 或 undefined，返回 null
  if (!hex) {
    return null
  }
 ...
 * 将 HSL 格式转换为 Hex
 */
export function hslToHex(hsl: HSLValue | null): string {
  // 如果输入为 null，返回默认颜色
  if (!hsl) {
    return '#000000'
  }
